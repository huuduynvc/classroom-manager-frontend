...
import { getTeachersOfClass, getStudentsOfClass, getClassMember } from "./peopleThunk";
...
export const peopleSlice = createSlice({
  ...
  extraReducers: {
    ...
    [getClassMember.pending as any]: (state) => {
      state.loading = true;
    },
    [getClassMember.rejected as any]: (state, action: any) => {
      state.loading = false;
      state.error = action.payload?.error;
    },
    [getClassMember.fulfilled as any]: (state, action) => {
      state.loading = false;
      state.list = action.payload.data;
    },
  },
